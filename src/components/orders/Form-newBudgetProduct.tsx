...
import { BudgetProduct } from "@/schemas/Orders";

interface FormNewBudgetProductProps {
    addProduct: (product: BudgetProduct) => void;
}

export const FormNewBudgetProduct: React.FC<FormNewBudgetProductProps> = ({ addProduct }) => {
    const [open, setOpen] = useState(false);
    const [searchQuery, setSearchQuery] = useState("");
    const [products] = useState<BudgetProduct[]>([
    ...
    const handleProductClick = (product: BudgetProduct) => {
        addProduct(product);
        setOpen(false);
    };
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button className="bg-amber-700 hover:bg-amber-800 text-white flex items-center">
                    Adicionar produtos <Plus size={16} className="ml-2" />
                </Button>
            </DialogTrigger>
            <DialogContent className="bg-white dark:bg-[#202020] dark:text-white">
                <DialogHeader>
                    <DialogTitle>Adicionando produtos ao orçamento</DialogTitle>
                    <DialogDescription>
